...
from src.pipeline.Detect_object.Mask_RCNN import MaskRCNN
from src.utils.logger import Logger

# --- CONFIG ---
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
PROJECT_NAME = "Object Detection"
OUTPUT_DIR = "./models/mask_rcnn_logs"
...
def train():
    ...
    wandb.init(project=PROJECT_NAME, name=f"MaskRCNN-{datetime.now().strftime('%m%d-%H%M')}")
    ...
    # Model với số lượng class của LVIS
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for images, targets in pbar:
            ...
            # Trong hàm train(), chỗ tính loss:
            loss_dict = model(images, targets)

            # Kiểm tra nếu là dict thì mới cộng (đề phòng lỗi trả về tuple)
            if isinstance(loss_dict, dict):
                losses = loss_dict["loss_classifier"] + loss_dict["loss_box_reg"]
            else:
                # Nếu lỡ trả về tuple (khi không ở mode training)
                raise RuntimeError("Model không trả về dict loss. Hãy kiểm tra model.train()")
            # losses = loss_dict["loss_classifier"] + loss_dict["loss_box_reg"]
            optimizer.zero_grad()
            losses.backward()
            optimizer.step()
        ...
        torch.save(model.state_dict(), os.path.join(OUTPUT_DIR, "MaskRCNN_latest.pth"))
